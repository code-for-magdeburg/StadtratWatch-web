...
export function calcVotingSuccessRateOfFaction(faction: RegistryFaction, sessions: SessionInput[]): number | null {

  const votingsSuccess = sessions
    .flatMap(session => {
      const persons = session.config.names.filter(name => name.faction === faction.name).map(name => name.name);
      const successfulVotings = session.votings.filter(voting => isPersonsVotingSuccessful(persons, voting));
...
export function calcVotingSuccessRateHistoryOfFaction(faction: RegistryFaction, sessions: SessionInput[]): HistoryDataPoint[] {
...
export function calcVotingSuccessRateOfParty(party: RegistryParty, sessions: SessionInput[]): number | null {
...
export function calcVotingSuccessRateHistoryOfParty(party: RegistryParty, sessions: SessionInput[]): HistoryDataPoint[] {
...
export function calcVotingSuccessRateOfPerson(person: RegistryPerson, sessions: SessionInput[]): number | null {
...
function calcPersonsVotingResult(persons: string[], voting: SessionScanItem): VotingResult {
