...
const magdeburg = JSON.parse(fs.readFileSync('../data/Magdeburg.json', 'utf8')) as ScrapedSession;
...
const agendaItems = magdeburg.agenda_items
  ...
  .filter(agendaItem => meetings.some(meeting => meeting.original_id === agendaItem.meeting_id));
const papers = magdeburg.papers.filter(
  paper => agendaItems.some(agendaItem => agendaItem.paper_original_id === paper.original_id)
);
const files = magdeburg.files.filter(
  file => papers.some(paper => paper.original_id === file.paper_original_id)
);

const scrapedMeetings = defineCollection({
  loader: () => meetings.map(meeting => ({ ...meeting, id: `${meeting.original_id}` })),
  schema: z.custom<ScrapedMeeting>()
});

const scrapedAgendaItems = defineCollection({
  loader: () => agendaItems.map(agenda_item => ({ ...agenda_item, id: `${agenda_item.meeting_id}-${agenda_item.key}` })),
  schema: z.custom<ScrapedAgendaItem>()
});

const scrapedPapers = defineCollection({
  loader: () => papers.map(paper => ({ ...paper, id: `${paper.original_id}` })),
