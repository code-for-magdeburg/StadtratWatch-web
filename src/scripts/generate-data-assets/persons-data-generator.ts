import { PersonVotingComparison, PersonDetailsDto, PersonsForcesDto, PersonStatsHistoryDto, PersonLightDto, PersonSpeechDto, PersonVoteDto, SessionDetailsDto, SessionVotingDto, VoteResult } from '@scope/interfaces-web-assets';
...
export class PersonsDataGenerator {


  public generatePersonsData(registry: Registry, sessions: SessionDetailsDto[]): GeneratedPersonsData {
    ...
    const persons = registry.persons.map<PersonDetailsDto>(person => {
      ...
      const votingMatrix = this.calcVotingMatrix(registry, votings, person.id);
      const votingAttendance = this.calcVotingAttendance(relevantSessions, person.id);
      const votingSuccess = calcPersonVotingSuccess(relevantSessions, person.id);
      const abstentionStats = this.calcAbstentionStats(relevantSessions, person.id);
      ...
      return {
        ...
        statsHistory: this.calcStatsHistory(relevantSessions, person.id),
        speeches
      };
    });
    ...
    const personsLight = persons.map<PersonLightDto>(person => ({
      ...
      speakingTime: this.calcSpeakingTime(person),
    }));
  ...
  private calcVotingMatrix(registry: Registry, votings: SessionVotingDto[], personId: string): PersonVotingComparison[] {
    ...
    return otherPersons.map<PersonVotingComparison>(otherPerson => ({
      ...
      comparisonScore: this.calcVotingComparisonScore(votings, personId, otherPerson.id)
    }));

  }


  private calcVotingComparisonScore(votings: SessionVotingDto[], personId: string,
  ...
  private calcVotingAttendance(sessions: SessionDetailsDto[], personId: string): number {
  ...
  private calcAbstentionStats(sessions: SessionDetailsDto[], personId: string): {
  ...
  private calcStatsHistory(sessions: SessionDetailsDto[], personId: string): PersonStatsHistoryDto {

    return {
      votingAttendance: sessions.map(session => ({
        date: session.date,
        value: this.calcVotingAttendance(sessions.filter(s => s.date <= session.date), personId)
      })),
      ...
      abstentionRate: sessions.map(session => ({
        date: session.date,
        value: this.calcAbstentionStats(sessions.filter(s => s.date <= session.date), personId).abstentionRate
      }))
    };
  ...
  private calcSpeakingTime(person: PersonDetailsDto): number {
