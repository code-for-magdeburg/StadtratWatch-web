...
export class FractionParticipationRateComponent implements OnChanges {
  ...
  public chartData: ChartConfiguration['data'] = FractionParticipationRateComponent.initialChartData();
  public chartOptions: ChartConfiguration['options'] = FractionParticipationRateComponent.initialChartOptions();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['fraction']) {
      this.chartData.datasets = [
        {
          data: this.fraction.statsHistory.participationRate.map(value => value.value),
          fill: 'origin',
        }
      ];
      this.chartData.labels = this.fraction.statsHistory.participationRate.map(value => value.date);
    }
